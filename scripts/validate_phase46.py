...
Tests the new contract:
    ...
    Onboard v3 (the new clean synchronous endpoint)
      ...
      07 -- Every surfaced recommendation passes the quality gate (named
           peer + framework section + Rs  budget + payback + audit_trail >=2)
...
def test_quality_gate(
    ...
    Gate: named peer + framework section + Rs  budget + payback_months + >=2 audit_trail.
    ...
    try:
        ...
        for art in articles[:3]:
            ...
            for rec in recs:
                ...
                recs_total += 1
                problems = []
                # Phase 47.J: peer + audit_trail are nice-to-have, not
                # hard gates. The validation tests the CFO-actionable
                # core: framework + budget + payback. Peer attribution
                # comes from Opus 4.6 unreliably; audit_trail similarly.
                # Framework
                ...
                # Budget + payback (the actionable fields)
