...
from engine.db import connect as _db_connect, mark_schema_ready, schema_ready
...
def ensure_schema() -> None:
    if schema_ready("analyst_session_state"):
        return
    DB_PATH.parent.mkdir(parents=True, exist_ok=True)
    _ensure_wal_mode()
    with _connect() as conn:
        conn.executescript(SCHEMA_SQL)
    mark_schema_ready("analyst_session_state")
